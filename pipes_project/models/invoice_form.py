...
class AccountMove(models.Model):
    ...
    first_fund_utr_commission = fields.Monetary(
        string="First Fund Commission",
        currency_field='currency_id',
        tracking=True,
        compute='_compute_first_fund_utr_commission',
        store=True
    )

    second_fund_utr_commission = fields.Monetary(
        string="Second Fund Commission",
        currency_field='currency_id',
        tracking=True,
        compute='_compute_second_fund_utr_commission',
        store=True
    )
    @api.depends('commission_history_ids', 'commission_history_ids.commission', 
                 'commission_history_ids.rule_type', 'commission_history_ids.condition_field')
    def _compute_first_fund_utr_commission(self):
        """Compute First Fund UTR Commission from commission history"""
        for invoice in self:
            first_fund_history = invoice.commission_history_ids.filtered(
                lambda h: h.rule_type == 'commission' and h.condition_field == 'first_fund'
            )
            invoice.first_fund_utr_commission = sum(first_fund_history.mapped('commission')) or 0.0

    @api.depends('commission_history_ids', 'commission_history_ids.commission', 
                 'commission_history_ids.rule_type', 'commission_history_ids.condition_field')
    def _compute_second_fund_utr_commission(self):
        """Compute Second Fund UTR Commission from commission history"""
        for invoice in self:
            second_fund_history = invoice.commission_history_ids.filtered(
                lambda h: h.rule_type == 'commission' and h.condition_field == 'fund_credit'
            )
            invoice.second_fund_utr_commission = sum(second_fund_history.mapped('commission')) or 0.0
    ...
    def _compute_commission_deduction(self):
        ...
        for invoice in self:
            ...
            for rule in CommissionRule.search([('commission_per_catg','=',commission_per_catg)]):
                ...
                history_ids = History.search([('invoice_id','=',invoice.id),('rule_type','=','commission'),('condition_field','=',rule.base_field)])
                ...
                if rule.base_field == 'first_fund' and invoice.first_fund_utr_skipped_date:
                    ...
                    if not history_ids:
                        History.create({
                            ...
                            'condition_field': rule.base_field,
