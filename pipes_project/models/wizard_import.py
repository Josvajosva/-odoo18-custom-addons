...
class ImportCrmData(models.TransientModel):
    ...
    #     if partner_by_app_id:
    #         if not partner_by_app_id.name.endswith(application_id):
    #             partner_by_app_id.name = f"{partner_by_app_id.name}-{application_id}"
    #         return partner_by_app_id.id
    ...
    #         if not partner.name.endswith(application_id):
    #             partner.name = f"{partner.name}-{application_id}"
    ...
    #     formatted_name = f"{customer_name}-{application_id}"
    ...
    #         _logger.info("Created new partner: %s with Application ID: %s",
    #                     formatted_name, application_id)
    #         return new_partner.id
    ...
    def create_or_find_partner(self, values, application_id):
        """Create or find customer using application ID first. Name format: 'CustomerName || APPID'"""
        try:
            customer_raw = values.get('customer', '') or ''
            customer_name = customer_raw.strip() or 'Customer'
            formatted_name = f"{customer_name} || {application_id}"

            partner = self.env['res.partner'].search([('application_id', '=', application_id)], limit=1)
            if partner:
                if formatted_name not in partner.name:
                    try:
                        partner.name = formatted_name
                    except Exception:
                        _logger.warning("Could not update partner name for partner id %s", partner.id)
                return partner.id

            caste_mapping = {'bc':'bc','mbc':'mbc','sc':'sc','st':'st','oc':'oc','other':'other'}
            ...
            partner_data = {
                ...
                'caste': caste_mapping.get(caste_value, 'other'),
                'gender': gender_mapping.get(gender_value, 'other'),
                'street': f"{values.get('village','')}, {values.get('block','')}".strip(', '),
                'city': values.get('district', '').strip(),
                'phone': values.get('mobile','').strip(),
                'application_id': application_id,
                'customer_rank': 1,
            }
            ...
            new_partner = self.env['res.partner'].create(partner_data)
            _logger.info("Created partner %s (id %s) for application_id %s", new_partner.name, new_partner.id, application_id)
            return new_partner.id

        except Exception:
            _logger.exception("Error in create_or_find_partner for application_id %s, values: %s", application_id, values)
            return False
    ...
    def find_or_create_dealer(self, dealer_name):
        """Find or create dealer and ensure 'Dealer' tag/category is set."""
        try:
            ...
            dealer_tag = category_model.search([('name', '=', 'Dealer')], limit=1)
            if not dealer_tag:
                dealer_tag = category_model.create({'name': 'Dealer'})

            dealer = partner_model.search([('name', '=ilike', dealer_name)], limit=1)
            if dealer:
                if dealer_tag.id not in dealer.category_id.ids:
                    dealer.write({'category_id': [(4, dealer_tag.id)]})
                if hasattr(dealer, 'is_dealer'):
                    try:
            ...
            dealer_data = {
                'name': dealer_name,
                'company_type': 'person',
                'category_id': [(6, 0, [dealer_tag.id])],
            }
            if hasattr(self.env['res.partner'], 'is_dealer'):
                dealer_data['is_dealer'] = True

            new_dealer = partner_model.create(dealer_data)
            _logger.info("Created dealer partner %s (id %s) with Dealer category", new_dealer.name, new_dealer.id)
            return new_dealer.id

        except Exception:
            _logger.exception("Error in find_or_create_dealer for dealer_name: %s", dealer_name)
            return False
...
class ImportInvoiceData(models.TransientModel):
    ...
    def create_invoice(self, values):
        ...
        fund_credit = self.parse_float(values.get('fund credit', '0') or values.get('fund_credit', '0') or values.get('fund_credited', '0'))
        first_fund = self.parse_float(values.get('first fund', '0') or values.get('first_fund', '0'))
        ...
        if hasattr(self.env['account.move'], 'fund_credit'):
            invoice_data['fund_credit'] = fund_credit
            _logger.info("Set fund_credit: %s for Application ID: %s", fund_credit, application_id)

        if hasattr(self.env['account.move'], 'first_fund'):
            invoice_data['first_fund'] = first_fund
            _logger.info("Set first_fund: %s for Application ID: %s", first_fund, application_id)
        ...
        text_fields_mapping = {
            ...
            'utr_number': ['utr number', 'utr_number'],
        }
        ...
        try:
            if existing_invoice:
                ...
                _logger.info("Successfully UPDATED invoice for Application ID: %s with invoice value: %s, fund_credit: %s, first_fund: %s", 
                            application_id, invoice_value, fund_credit, first_fund)
                ...
                _logger.info("Successfully CREATED new invoice for Application ID: %s with invoice value: %s, fund_credit: %s, first_fund: %s", 
                            application_id, invoice_value, fund_credit, first_fund)
                return invoice
