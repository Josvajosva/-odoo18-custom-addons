...
class PurchaseRequestLine(models.Model):
    ...
    product_on_hand_qty = fields.Float(
        ...
        store=True,
        help="The current available quantity of the product in stock."
    )

    needed_qty = fields.Float(
        string="Needed Qty", 
        tracking=True,
        compute="_compute_needed_qty", 
        store=True,
        readonly=False
    )
    ven_id = fields.Many2one('res.partner', string="Vendor")
    ven_domain_ids = fields.Many2many('res.partner')
            
    @api.depends('needed_qty', 'product_id', 'product_on_hand_qty')
    def _compute_estimated_cost(self):
        for record in self:
            # Get unit cost from PR line or fallback to standard_price
            unit_price = record.product_id.standard_price
            record.estimated_cost = unit_price * record.needed_qty if record.needed_qty else 0.0
            record.purchased_qty = abs(record.product_on_hand_qty - record.needed_qty)


    # @api.depends('product_id', 'request_id.origin')
    # def _compute_needed_qty(self):
    #     for line in self:
    #         needed_qty = 0.0
    #         if line.request_id.origin:
    #             sale_order = self.env['sale.order'].search([
    #                 ('name', '=', line.request_id.origin)
    #             ], limit=1)
    #             if sale_order:
    ...
    #                                 needed_qty += so_line.product_uom_qty * bom_line.product_qty
    #         line.needed_qty = needed_qty

    @api.depends('product_id', 'request_id.origin', 'product_qty', 'purchased_qty')
    def _compute_needed_qty(self):
        for line in self:
            product = line.product_id
            purchased_qty = line.purchased_qty or 0.0
            onhand_qty = product.qty_available or 0.0

            if not line.request_id.origin:
                line.needed_qty = purchased_qty + onhand_qty
                            ...
                            if bom:
                                for bom_line in bom.bom_line_ids:
                                    if bom_line.product_id == product:
                                        bom_qty += so_line.product_uom_qty * bom_line.product_qty
                    line.needed_qty = bom_qty + onhand_qty
    ...
    def _compute_product_on_hand_qty(self):
        for rec in self:
            if rec.product_id and rec.company_id:
                rec.product_on_hand_qty = rec.product_id.with_context(
                    company_id=rec.company_id.id
                ).qty_available
            else:
                rec.product_on_hand_qty = 0.0
    ...
    def _compute_qty_to_buy(self):
        for pr in self:
            qty_to_buy = abs(sum(pr.mapped("needed_qty")) - sum(pr.mapped("product_on_hand_qty")))
    ...
    def _compute_purchased_qty(self):
        for rec in self:
            rec.purchased_qty = abs(rec.product_on_hand_qty - rec.needed_qty)
